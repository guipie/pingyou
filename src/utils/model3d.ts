...
/**
 * convertFileSrc 的 Windows 安全版：
 * 1) 统一把反斜杠换成正斜杠（避免 Tauri asset protocol 对 scope glob 匹配失败）
 * 2) 返回 http://asset.localhost/<encoded> 形式的 URL
 */
...
/**
 * 判断给定的路径是否是 3D 模型文件（不是目录）
 */
const MODEL_FILE_EXT_RE = /\.(?:glb|gltf|vrm|fbx)$/i;
function isModelFile(path: string) {
  return MODEL_FILE_EXT_RE.test(path);
}
...
class Model3d {
  ...
  // 自适应后的模型高度（世界单位），用于按比例计算蹲/跳幅度
  private fitHeight = 2;
  // 垂直位置平滑值（呼吸/下蹲参与平滑，跳跃偏移直接叠加保证弹跳干脆）
  private smoothedPosY: number | null = null;
  // 实例化赋值
  ...
  public async load(path: string, canvasId?: string): Promise<LoadResult> {
    ...
    // 关键修复：如果 path 是目录（store 里自定义模型通常存的是目录），
    //          自动在目录下找 glb/gltf/vrm/fbx 文件，否则直接传目录给
    //          GLTFLoader.load 会 asset protocol 直接 403
    const modelFile = isModelFile(path)
      ? path
      : await this.findModelFileInDir(path);

    this.model = await this.load3DModel(modelFile);
  ...
  /**
   * 在指定目录下递归扫描 3D 模型文件（glb/gltf/vrm/fbx）。
   * 支持任意层级嵌套（zip 解压后可能套多层目录如 包名/模型名/模型名.glb）。
   * 如果配置里写了 file，优先用配置的；否则取扫描到的第一个。
   */
  private async findModelFileInDir(dir: string): Promise<string> {
    if (this.config?.file) {
      const target = join(dir, this.config.file);
      if (isModelFile(target)) return target;
    }
    // BFS 递归搜索：不限层数
    ...
    // 找不到时返回原目录路径，让 loader 抛出清晰错误
    return dir;
  }

  public async loadConfigs() {
    const modelsPath = await resolveResource("assets/models/model3d");
    const configPath = await resolveResource("assets/models/model3d.json");
    const configs = await this.readConfigs(configPath);
    const configByFile = new Map(configs.map(config => [config.file, config]));
    const files = await this.findModelFiles(modelsPath);

    return files.map(file => configByFile.get(file) ?? { file });
  }

  private async readConfigs(path: string): Promise<Model3dConfig[]> {
    try {
      const configs = JSON5.parse(await readTextFile(path));
  ...
  private async findModelFiles(path: string): Promise<string[]> {
    ...
    for (const entry of entries) {
      if (!entry.isDirectory && /\.(?:glb|gltf|vrm|fbx)$/i.test(entry.name)) {
  ...
  private loadGltf(path: string) {
    // 关键修复 (gltf 外部资源)：gltf/gltf 的 textures/bin 文件是相对路径，
    // three.js 默认会把它们相对于 window.location (http://localhost:1420) 去 fetch，
    // 导致 404。这里通过 LoadingManager 的 URLModifier 把相对 URL 映射到
    // 「模型文件所在目录 + 相对路径」的 asset URL。
    const modelDirFs = path.split("\\").join("/").replace(/[^/]+$/, "");
    const manager = new LoadingManager();
    manager.setURLModifier((url) => {
      // 绝对 http(s)/asset/data/blob/… 直接返回
      if (/^(?:[a-z][a-z0-9+.-]*:|\/|#)/i.test(url)) return url;
      const normalized = url.replace(/^\.\//, "");
      return toAssetUrl(`${modelDirFs}${normalized}`);
    });
  ...
  public fitModel() {
    ...
    // 记录自适应后的模型高度，供弹跳/下蹲幅度按比例计算
    this.fitHeight = targetHeight;
  ...
  // 键盘整体按下状态：按下时下蹲，抬起瞬间给向上初速度形成弹跳
  public setKeyPressed(pressed: boolean) {
    if (pressed === this.keyPressed) return;

    this.keyPressed = pressed;

    if (!pressed) {
      // 抬起瞬间起跳（轻微弹跳，峰值约 4% 模型高）
      this.jumpVelocity = this.fitHeight * 1.1;
    }
  }
  ...
  private tick() {
    ...
    // —— 键盘蹲/跳物理 ——
    // 按下：轻微下蹲（目标约 5% 模型高）；抬起：setKeyPressed 已给向上初速度，这里用重力回落
    const crouchTarget = this.keyPressed ? -this.fitHeight * 0.02 : 0;
    this.crouchOffset += (crouchTarget - this.crouchOffset) * Math.min(1, delta * 25);

    if (this.jumpVelocity !== 0 || this.jumpOffset > 0) {
      this.jumpVelocity -= this.fitHeight * 14 * delta;
    ...
    if (controls) {
      // 垂直位置：呼吸 + 下蹲参与平滑，跳跃偏移直接叠加（保证弹跳干脆不被抻平）
      const targetY = controls.basePosition.y + Math.sin(elapsed * 2) * 0.025 + this.crouchOffset;
      this.smoothedPosY = this.smoothedPosY === null
        ? targetY
        : this.smoothedPosY + (targetY - this.smoothedPosY) * Math.min(1, delta * 12);
      this.model.position.set(
        controls.basePosition.x,
        this.smoothedPosY + this.jumpOffset,
        controls.basePosition.z,
      );

      // 形变：下蹲时压扁，起跳/落地时拉伸/回压，强化打击感
      const crouchRatio = this.fitHeight > 0 ? -this.crouchOffset / this.fitHeight : 0;
      const stretchRatio = this.jumpOffset > 0
        ? Math.max(-0.06, Math.min(0.06, this.jumpVelocity / (this.fitHeight * 12)))
        : 0;
      this.model.scale.lerp(
        controls.baseScale.clone().multiply(new Vector3(
          1 + crouchRatio * 0.5 - stretchRatio * 0.3,
          1 - crouchRatio * 0.8 + stretchRatio * 0.6,
          1 + crouchRatio * 0.5 - stretchRatio * 0.3,
        )),
        0.35,
      );

      // 旋转：鼠标左右驱动偏航（Y）、鼠标上下驱动俯仰（X，叠加手柄摇杆）
      this.model.quaternion.slerp(
        controls.baseQuaternion.clone().multiply(
          new Quaternion().setFromEuler(new Euler(
            this.lookTarget.y * -0.45 + this.stickTilt.y * 0.2,
            this.lookTarget.x * 0.85,
    ...
    if (head) {
      head.quaternion.slerp(
        (controls?.headBaseQuaternion ?? new Quaternion()).clone().multiply(
          new Quaternion().setFromEuler(new Euler(this.lookTarget.y * -0.5, this.lookTarget.x * 0.55, 0)),
        ),
        0.14,
      );
    }
