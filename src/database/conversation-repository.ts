...
class ConversationRepository extends BaseRepository<TauriAIConversation> {
  ...
  /**
   * 按 id 取单个会话。
   * 必须与 getConversations 一样做「解密 apiKey + 还原 JSON 字段」，
   * 否则 provider 会是原始字符串、config/options 是字符串，直接把会话用坏。
   */
  async getConversationById(id: string): Promise<TauriAIConversation | null> {
    const db = await this.getDB();
    const rows = await db.select<any[]>(
