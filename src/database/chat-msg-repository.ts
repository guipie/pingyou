...
/** options 列里顺带承载的多模态链路元数据（没有独立列，跟 options 一起落库） */
interface ChatMsgOptionsExtras {
...
/**
 * 把消息打包成 options 列的值。
 * 图片描述 / 识图模型这两个字段表里没有独立列，塞进 options 一起存，
 * 避免为了两个可选字段就动表结构。
 */
function packMessageOptions(msg: TauriAIChatMessage): string {
  const base = msg.options && typeof msg.options === "object" ? msg.options : {};
  const payload: Record<string, unknown> = { ...base };
  if (msg.imageDescription) payload.imageDescription = msg.imageDescription;
  if (msg.visionModel) payload.visionModel = msg.visionModel;
  return Object.keys(payload).length > 0 ? JSON.stringify(payload) : "";
}

/** DB 行 → 消息对象：还原 file/options，并把多模态元数据摊平回顶层字段 */
function mapMessageRow(row: any): TauriAIChatMessage {
  const options = safeJsonParse<ChatMsgOptionsExtras | undefined>(row.options, undefined);
  return {
    ...row,
...
class ChatMsgRepository extends BaseRepository<TauriAIChatMessage> {
  ...
  async saveMessage(conversationId: string, msg: TauriAIChatMessage): Promise<boolean> {
    ...
    const result = await db.execute(
      ...
      [msg.id, conversationId, msg.role, msg.question, msg.answer, (msg.error ? String(msg.error) : ""), (msg.file ? JSON.stringify(msg.file) : ""), packMessageOptions(msg), msg.timestamp, msg.timestampAnswer],
    );
