...
import { ollamaEngine } from "./composables/useOllamaEngine";
...
onMounted(async () => {
  ...
  // 本地大模型：启动时自动拉起 Ollama 引擎，并把已下载的模型同步为可用供应商。
  // 不阻塞启动流程（引擎拉起/组件下载可能耗时），失败也不影响应用其它功能。
  try {
    await ollamaEngine.ensureEngine();
  } catch (e) {
    console.error("[startup] 本地引擎自动拉起失败:", e);
  }
});
...
// 主题同步到 html.dark：气泡窗(winmsg)/输入窗(winchat)/主窗口都是独立 webview，
// 只靠设置页那个 watcher 它们永远是浅色，气泡的深浅色适配会失效。
